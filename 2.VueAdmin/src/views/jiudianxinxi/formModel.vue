		...
		<el-dialog v-model="formVisible" :title="formTitle" width="80%" destroy-on-close :fullscreen='false'>
			<el-form class="formModel_form" ref="formRef" :model="form" label-width="$template2.back.add.form.base.labelWidth" :rules="rules">
				<el-row>
					...
					<el-col :span="24">
						<el-form-item label="一晚价格" prop="yiwanjiage">
							<el-input class="list_inp" v-model.number="form.yiwanjiage" placeholder="一晚价格"
								 type="text" 								:readonly="!isAdd||disabledForm.yiwanjiage?true:false" />
						</el-form-item>
					</el-col>
					...
					<el-col :span="24">
						<el-form-item label="房间状态" prop="fangjianzhuangtai">
							<el-select
								class="list_sel"
								:disabled="!isAdd||disabledForm.fangjianzhuangtai?true:false"
								v-model="form.fangjianzhuangtai" 
								placeholder="请选择房间状态"
								>
								<el-option v-for="(item,index) in fangjianzhuangtaiLists" :label="item"
									:value="item"
									>
								</el-option>
							</el-select>
...
<script setup>
	...
	//房间状态列表
	const fangjianzhuangtaiLists = ref([])
	//methods
	...
	const getInfo = ()=>{
		context?.$http({
			...
			res.data.data.fangneisheshi = res.data.data.fangneisheshi?(res.data.data.fangneisheshi.replace(reg,'../../../cl6751742/file')):'';
	...
	const init=(formId=null,formType='add',formNames='',row=null,table=null,statusColumnName=null,tips=null,statusColumnValue=null)=>{
		...
		if(formType == 'add'){
			...
			for(let x in row){
				...
				if(x=='fangjianzhuangtai'){
					form.value.fangjianzhuangtai = row[x];
					disabledForm.value.fangjianzhuangtai = true;
					continue;
				}
				if(x=='clicktime'){
			...
			form.value.fangjianzhuangtai='未预定'
		...
		fangjianzhuangtaiLists.value = "已预定,未预定".split(',')
	}
	//初始化
	//声明父级调用
	defineExpose({
