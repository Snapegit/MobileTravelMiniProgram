...
<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback">
	<view class="content">
		<view :style='{"width":"100%","padding":"0 0 0 0","position":"relative","background":"#ffffff","height":"100%"}'>
			...
			<view :style='{"width":"100%","flexWrap":"wrap","background":"#ffffff","justifyContent":"space-between","display":"flex","height":"auto"}'>
				<scroll-view scroll-x="true" class="category-one" :style='{"padding":"0px 24rpx 0px 24rpx","margin":"20rpx 0 20rpx 0","whiteSpace":"nowrap","background":"#ffffff","display":"flex","width":"100%","height":"auto"}'>
					<view :class='categoryName === item.jiudianleixing ? "active" : ""' class="tab" v-for="(item,index) in categoryList" :key="index" @tap="categoryClick(item.jiudianleixing)">{{item.jiudianleixing}}</view>
				</scroll-view>
			...
			<view class="uni-product-list" :style='{"padding":"24rpx 24rpx 24rpx 24rpx","margin":"40rpx 0 0 0","alignItems":"flex-start","flexWrap":"wrap","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
				<view @tap="onDetailTap(product)" class="uni-product" :style='{"boxShadow":"0 6rpx 12rpx #eeeeee","margin":"0 0 40rpx 0","backgroundColor":"#ffffff","borderRadius":"8rpx","flexWrap":"wrap","textAlign":"center","display":"flex","width":"48%","justifyContent":"center","height":"auto"}' v-for="(product,index) in list" :key="index">
					<view class="uni-product-title" :style='{"padding":"0px 20rpx 0px 20rpx","margin":"0 0 0px 0","whiteSpace":"nowrap","color":"#333333","textAlign":"left","overflow":"hidden","borderRadius":"8rpx","width":"100%","lineHeight":"72rpx","fontSize":"28rpx","textOverflow":"ellipsis","order":"2","height":"72rpx"}'>{{product.fangjianmingcheng}}</view>
					<image :style='{"border":"0px solid #eeeeee","padding":"0 0 0 0","margin":"0 0 0px 0","objectFit":"cover","borderRadius":"8rpx","display":"inline-block","width":"100%","height":"220rpx"}' mode="aspectFill" class="uni-product-image" v-if="preHttp(product.fangjiantupian)" :src="product.fangjiantupian.split(',')[0]"></image>
					<image :style='{"border":"0px solid #eeeeee","padding":"0 0 0 0","margin":"0 0 0px 0","objectFit":"cover","borderRadius":"8rpx","display":"inline-block","width":"100%","height":"220rpx"}' mode="aspectFill" class="uni-product-image" v-else :src="product.fangjiantupian?baseUrl+product.fangjiantupian.split(',')[0]:''"></image>
					<view class="uni-product-title" :style='{"padding":"0px 20rpx 0px 20rpx","margin":"0 0 0px 0","whiteSpace":"nowrap","color":"#333333","textAlign":"left","overflow":"hidden","borderRadius":"8rpx","width":"100%","lineHeight":"72rpx","fontSize":"28rpx","textOverflow":"ellipsis","order":"2","height":"72rpx"}'>一晚价格:{{product.yiwanjiage}}</view>
					<view :style='{"padding":"0","margin":"8rpx 0 0 0","display":"flex","width":"100%","justifyContent":"space-between","height":"auto","order":"3"}'>
						<view :style='{"border":"0px solid #dcf8f5","padding":"0px 20rpx 20rpx 20rpx","borderRadius":"60rpx","display":"flex"}' v-if="(userid && isAuth('jiudianxinxi','修改')) || (!userid && isAuthFront('jiudianxinxi','修改'))" @click.stop="onUpdateTap(product.id)">
						...
						<view :style='{"border":"0px solid #f9dede","padding":"0px 20rpx 20rpx 20rpx","borderRadius":"60rpx","display":"flex"}' v-if="(userid && isAuth('jiudianxinxi','删除')) || (!userid && isAuthFront('jiudianxinxi','删除'))" @click.stop="onDeleteTap(product.id)">
		...
		<button :style='{"border":"0","boxShadow":"0 2rpx 12rpx #cccccc","color":"#333333","bottom":"120rpx","right":"120rpx","outline":"none","borderRadius":"100%","background":"#75ebde","width":"80rpx","lineHeight":"80rpx","fontSize":"28rpx","position":"fixed","height":"80rpx","zIndex":"999"}' v-if="userid && isAuth('jiudianxinxi','新增')" class="add-btn" @click="onAddTap()">新增</button>
		<button :style='{"border":"0","boxShadow":"0 2rpx 12rpx #cccccc","color":"#333333","bottom":"120rpx","right":"120rpx","outline":"none","borderRadius":"100%","background":"#75ebde","width":"80rpx","lineHeight":"80rpx","fontSize":"28rpx","position":"fixed","height":"80rpx","zIndex":"999"}' v-if="!userid && isAuthFront('jiudianxinxi','新增')" class="add-btn" @click="onAddTap()">新增</button>
	</view>
...
<script>
	export default {
		...
		async onShow() {
            ...
            let res = {};
            if(this.userid) {
                res = await this.$api.page('jiudianleixing', {page:1,limit:100});
            } else {
                res = await this.$api.list('jiudianleixing', {page:1,limit:100});
            }
			res.data.list.splice(0,0,{id:0,jiudianleixing:'全部'})
		...
		methods: {
			...
			async upCallback(mescroll) {
				...
				if(this.categoryName!='全部'){
					params.jiudianleixing = '%' + this.categoryName + '%'
				}
